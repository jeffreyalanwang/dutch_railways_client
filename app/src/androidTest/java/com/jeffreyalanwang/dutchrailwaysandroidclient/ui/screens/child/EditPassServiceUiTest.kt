...
class EditPassServiceUiTest {
    ...
    fun `creating a new pass service should allow adding stops and selecting stations`() {
        ...
        composeTestRule.onNodeWithText("Add stop").performClick()
        composeTestRule.onNodeWithText("Add stop").performClick()

        // Select station for stop 0
        composeTestRule.onNodeWithTag("stop_station_0").performClick()
        composeTestRule.onNode(hasSetTextAction()).performTextInput("Amsterdam")
        composeTestRule.onNodeWithText("Amsterdam Centraal").performClick()
        
        // Set departure time for stop 0 to 10:00
        composeTestRule.onNodeWithTag("stop_departure_0").performClick()
        composeTestRule.onNodeWithContentDescription("Use keyboard").performClick()
        composeTestRule.onAllNodes(hasSetTextAction())[0].performTextReplacement("10")
        composeTestRule.onAllNodes(hasSetTextAction())[1].performTextReplacement("00")
        composeTestRule.onNodeWithText("OK").performClick()

        // Select Trainset
        composeTestRule.onNodeWithTag("trainset_selector").performClick()
        composeTestRule.onNodeWithText("SLT").performClick()
        
        // Select station for stop 1
        composeTestRule.onNodeWithTag("stop_station_1").performClick()
        composeTestRule.onNode(hasSetTextAction()).performTextInput("Rotterdam")
        composeTestRule.onNodeWithText("Rotterdam Centraal").performClick()
        
        // Set arrival time for stop 1 to 11:00
        composeTestRule.onNodeWithTag("stop_arrival_1").performClick()
        composeTestRule.onNodeWithContentDescription("Use keyboard").performClick()
        composeTestRule.onAllNodes(hasSetTextAction())[0].performTextReplacement("11")
        composeTestRule.onAllNodes(hasSetTextAction())[1].performTextReplacement("00")
        composeTestRule.onNodeWithText("OK").performClick()
    ...
    fun `editing an existing pass service should display correctly pre-filled data`() {
        ...
        composeTestRule.onNodeWithText("Edit train service").assertIsDisplayed()
        composeTestRule.onNodeWithTag("stop_station_0").assert(hasAnyDescendant(hasText("Den Haag Centraal")))
    }
    ...
    fun `reordering stops in edit mode should update their position in the list`() {
        ...
        composeTestRule.onNodeWithTag("stop_station_0").assert(hasAnyDescendant(hasText("Station 1")))
        composeTestRule.onNodeWithTag("stop_station_1").assert(hasAnyDescendant(hasText("Station 2")))
        ...
        // After reorder
        composeTestRule.onNodeWithTag("stop_station_0").assert(hasAnyDescendant(hasText("Station 2")))
        composeTestRule.onNodeWithTag("stop_station_1").assert(hasAnyDescendant(hasText("Station 1")))
    }
}
